...
from javax.swing import JPopupMenu, JMenuItem, SwingUtilities, Box, JTextArea, JMenuItem, JFrame, JPanel, JButton, JLabel, JTextField, JSplitPane, SwingConstants, JCheckBox, JScrollPane, BorderFactory, BoxLayout, JComboBox
...
class BurpExtender(IBurpExtender, IMessageEditorController, ITab, IContextMenuFactory):
    def registerExtenderCallbacks(self, callbacks):
        ...
        try:
            self.top_panel, self.payload_input, self.wordlist_input, self.level_input, self.timeout, self.risk_input, self.dalfox_options_input, self.tplmap_options_input, self.url_encode_single, self.url_encode_wordlist, self.rate_limit_single_input, self.rate_limit_wordlist_input, self.dalfox_timeout_input, self.tplmap_timeout_input, self.tplmap_level_input, self.commix_options_input, self.commix_level_input, self.commix_timeout_input, self.custom_payload_input, self.url_encode_custom, self.rate_limit_custom_input = self.ui_manager.create_top_panel(
                            self.launch_single_payload, self.launch_wordlist_payload, self.launch_sqlmap, self.launch_dalfox_action, self.launch_tplmap_action, self.launch_commix_action, self.launch_custom_payload)
    ...
    def add_request(self, invocation, mode):
        ...
        if not self.is_request_already_added(request_info):
            ...
            request_panel = RequestPanel(request_obj, self.main_panel, self)
            self.main_panel.add(request_panel)
    ...
    def launch_single_payload(self, payload, rate_limit):
        try:
            ...
            self.ui_manager.create_single_payload_frame(payload, self.requests, self._callbacks, self._helpers, rate_limit)
        except Exception as e:
    ...
    def launch_wordlist_payload(self, wordlist_path, rate_limit):
        try:
            ...
            self.ui_manager.create_wordlist_payload_frame(payloads, self.requests, self._callbacks, self._helpers, rate_limit)
        except Exception as e:
    ...
    def launch_custom_payload(self, payload, url_encode, rate_limit):
        rate_limit = int(rate_limit) if rate_limit.isdigit() else 0
        self.ui_manager.create_custom_payload_frame(payload, self.requests, self._callbacks, self._helpers, url_encode, rate_limit)
...
class LaunchedRequest:
    def __init__(self, request, param, payload, callbacks, helpers, mode):
        ...
        self.modified_request_bytes = None  # Store modified request bytes
    ...
    def modify_and_send_request(self):
        try:
            ...
            self.modified_request_bytes = new_request_bytes  # Store modified request bytes
        ...
        try:
            if self.param.getName() in request_body_json:
                ...
                try:
                    ...
                    pass  # If conversion fails, fallback to string
    ...
    def modify_non_json_request(self, new_request_bytes):
        try:
            ...
            else:  # Assume parameters by default
                new_request_bytes = self.modify_parameters(new_request_bytes)
...
class UIManager:
    ...
    def create_top_panel(launch_single_payload_action, launch_wordlist_payload_action, launch_sqlmap_action, launch_dalfox_action, launch_tplmap_action, launch_commix_action, launch_custom_payload_action):
        ...
        payload_input, url_encode_single, rate_limit_single_input = UIManager.add_payload_components(
            top_panel, constraints, 0, launch_single_payload_action)
        
        wordlist_input, url_encode_wordlist, rate_limit_wordlist_input = UIManager.add_wordlist_components(
        ...
        sqlmap_options_input, level_input, risk_input, timeout_input = UIManager.add_sqlmap_components(
            top_panel, constraints, 2, launch_sqlmap_action)
        
        dalfox_options_input, dalfox_timeout_input = UIManager.add_dalfox_components(
            top_panel, constraints, 3, launch_dalfox_action)

        tplmap_options_input, tplmap_level_input, tplmap_timeout_input = UIManager.add_tplmap_components(
            top_panel, constraints, 5, launch_tplmap_action)

        commix_options_input, commix_level_input, commix_timeout_input = UIManager.add_commix_components(
            top_panel, constraints, 7, launch_commix_action)
        
        custom_payload_input, url_encode_custom, rate_limit_custom_input = UIManager.add_custom_payload_components(
            top_panel, constraints, 9, launch_custom_payload_action)
        ...
        return (top_panel, payload_input, wordlist_input, level_input, timeout_input, risk_input, dalfox_options_input,
                ...
                custom_payload_input, url_encode_custom, rate_limit_custom_input)
    ...
    def add_payload_components(panel, constraints, row, action):
        ...
        return payload_input, url_encode_single, rate_limit_single_input
    ...
    def add_wordlist_components(panel, constraints, row, action):
        ...
        return wordlist_input, url_encode_wordlist, rate_limit_wordlist_input
    ...
    def add_custom_payload_components(panel, constraints, row, action):
        ...
        return custom_payload_input, url_encode_custom, rate_limit_custom_input
    ...
    @staticmethod
    def create_sort_panel(launched_request_panels, update_panel_callback):
        ...
        sort_button = JButton("Sort", actionPerformed=lambda event: UIManager.sort_launched_requests(launched_request_panels, sort_combobox.getSelectedItem(), update_panel_callback))
        constraints.gridx = 2
        sort_panel.add(sort_button, constraints)
        return sort_panel

    @staticmethod
    def sort_launched_requests(launched_request_panels, sort_key, update_panel_callback):
        launched_request_panels.sort(key=lambda panel: panel.get_sort_value(sort_key))
        update_panel_callback()

    @staticmethod
    def create_find_panel(launched_request_panels):
        ...
        find_button = JButton("Find", actionPerformed=lambda event: find_function(find_input.getText(), launched_request_panels))
        constraints.gridx = 2
    ...
    @staticmethod
    def create_single_payload_frame(payload, requests, callbacks, helpers, rate_limit):
        ...
        find_panel = UIManager.create_find_panel(launched_request_panels)
        sort_panel = UIManager.create_sort_panel(launched_request_panels, lambda: UIManager.update_main_panel(request_panels_container, launched_request_panels))
        ...
        request_panels_container.add(header_panel)
        scrollable_request_panels = JScrollPane(request_panels_container)
        scrollable_request_panels.setPreferredSize(Dimension(1000, 300))
        main_panel = JPanel()
        main_panel.setLayout(BoxLayout(main_panel, BoxLayout.Y_AXIS))
        main_panel.setPreferredSize(Dimension(1000, 450))
        main_panel.add(top_panel)
        main_panel.add(scrollable_request_panels)
        ...
        def process_requests():
            for request in requests:
                for param in (request.testing_params if request.mode == "params" else
                    ...
                    launched_request = LaunchedRequest(request, param, payload, callbacks, helpers, request.mode)
    ...
    @staticmethod
    def create_wordlist_payload_frame(payloads, requests, callbacks, helpers, rate_limit):
    ...
    def create_sqlmap_frame(level, timeout, risk, options, requests, callbacks, helpers, burp_extender, config):
        ...
        for request in requests:
            for param in (request.testing_params if request.mode == "params" else
                ...
                launched_request_panels.append(sqlmap_request_panel)
                request_panels_container.add(sqlmap_request_panel)

        scrollable_request_panels = JScrollPane(request_panels_container)
        scrollable_request_panels.setPreferredSize(Dimension(650, 300))
        main_panel = JPanel()
        main_panel.setLayout(BoxLayout(main_panel, BoxLayout.Y_AXIS))
        main_panel.setPreferredSize(Dimension(650, 350))
        main_panel.add(scrollable_request_panels)
        frame.add(main_panel, BorderLayout.NORTH)
        frame.add(bottom_panel, BorderLayout.CENTER)
        frame.setVisible(True)
    ...
    def create_dalfox_frame(options, timeout, requests, callbacks, helpers, burp_extender, config):
        ...
        for request in requests:
            for param in request.testing_params:
                ...
                request_panels_container.add(dalfox_request_panel)
    ...
    def create_tplmap_frame(options, level, timeout, requests, callbacks, helpers, burp_extender, config):
        ...
        for request in requests:
            for param in request.testing_params:
                ...
                request_panels_container.add(tplmap_request_panel)
    ...
    def create_commix_frame(options, level, timeout, requests, callbacks, helpers, burp_extender, config):
        ...
        for request in requests:
            for param in request.testing_params:
                ...
                launched_request_panels.append(commix_request_panel)
                request_panels_container.add(commix_request_panel)
    ...
    @staticmethod
    def create_custom_payload_frame(payload, requests, callbacks, helpers, url_encode, rate_limit):
        ...
        # Start background thread for request processing
        def process_requests():
            for request in requests:
                for param in request.testing_params:
                    ...
                    launched_request = LaunchedRequest(request, param, modified_payload, callbacks, helpers, request.mode)
                    launched_request_panel = LaunchedRequestPanel(launched_request, request_text_area, response_text_area)
                    launched_request.set_panel(launched_request_panel)
                    SwingUtilities.invokeLater(lambda: request_panels_container.add(launched_request_panel))
                    launched_request_panels.append(launched_request_panel)
...
def find_function(find, launched_request_panels):
    for launched_request_panel in launched_request_panels:
        if find.lower() in launched_request_panel.launchedRequest.body_str.lower():
            launched_request_panel.launchedRequest.find_result = "Yes"
            launched_request_panel.update_find("Found!")
        else:
            launched_request_panel.launchedRequest.find_result = "No"
            launched_request_panel.update_find("--")
...
class LaunchedRequestPanel(JPanel):
    ...
    def setupUI(self):
        ...
        title = Utils.fix_string_size(title, 250)
        self.request_label = JLabel(title)
        self.request_label.setFont(Font("Dialog", Font.BOLD, 18))
        param = self.launchedRequest.param.getName() if isinstance(self.launchedRequest.param, IParameter) else self.launchedRequest.param[0] if isinstance(self.launchedRequest.param, tuple) else self.launchedRequest.param
        param = Utils.fix_string_size(param, 100)
        self.param_label = JLabel(param)
        status = Utils.fix_string_size(self.launchedRequest.status_code, 100)
        self.status_label = JLabel(status)
        diff = Utils.fix_string_size(self.launchedRequest.diff, 100)
        self.diff_label = JLabel(diff)
        response_time = Utils.fix_string_size(self.launchedRequest.response_time, 100)
        self.time_label = JLabel(response_time)
        unusual_content = Utils.fix_string_size(self.launchedRequest.unusual_content, 100)
        self.content_label = JLabel(unusual_content)
        unusual_headers = Utils.fix_string_size(self.launchedRequest.unusual_headers, 100)
        self.headers_label = JLabel(unusual_headers)
        find_result = Utils.fix_string_size(self.launchedRequest.find_result, 100)
    ...
    def format_http_request(self, request_info, body):
...
class HeaderPanel(JPanel):
    ...
    def set_label_sizes(self):
        ...
        self.getComponent(0).setPreferredSize(Dimension(500, 30))  # First label (Request) gets a larger size
...
class SQLMapRequest:
    def __init__(self, request, param, level, timeout, risk, options, callbacks, helpers, burp_extender, config):
        ...
        self.risk = risk
        self.options = options  # Added options
        self._callbacks = callbacks
        self._helpers = helpers
        self.burp_extender = burp_extender
        self.panel = None
    ...
    def run_sqlmap(self):
        ...
        try:
            ...
            command = "timeout " + str(self.timeout) + "s "+ self.config['sqlmap']['command'] + " -r " + file_path + test_param + " --batch --level=" + str(self.level) \
                  + " --risk=" + str(self.risk) + flags + " --flush-session --ignore-stdin " + self.options  # Append options
            print("Command:", command)

            process = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            timer = threading.Timer(420, process.kill)  # Timer to kill the process after 420 seconds
            try:
                timer.start()
...
class CommixRequest:
    ...
    def run_commix(self):
        try:
            file_path = self.burp_extender.save_request_file(self.original_request.request, self.param)
            command = "timeout " + str(self.timeout) + "s " + self.config['commmix']['command'] + " -r " + file_path + " --level=" + str(self.level) + " -p " + self.param.getName() + " --batch --ignore-stdin " + self.options  # Append options
            print("Command:", command)
            process = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            stdout, stderr = process.communicate()
            self.result = stdout
...
class Utils:
    ...
    @staticmethod
    def fix_string_size(s, target_width):
        ...
        if text_width < target_width:
            space_width = fm.stringWidth(" ")
            num_spaces = (target_width - text_width) // space_width
            return s + ' ' * num_spaces
        elif text_width > target_width:
            for i in range(len(s)):
                truncated_text = s[:len(s) - i] + '...'
                truncated_text_width = fm.stringWidth(truncated_text)
                if truncated_text_width <= target_width:
    ...
    def check_unusual_content(body):
        return "Seems pretty weird to me"
